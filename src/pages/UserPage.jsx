...
function UserPage({ data,order }) {

    const priorityLables=["No priority","Low" ,"Medium" ,"High" ,"Urgent" ]
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px' }}>
      {Object.entries(groupedTickets).map(([userId, tickets]) => {
        // Sort tickets
        ...
        return (
          ...
          ><div style={{display:"flex"}}>
            <h3 style={{ textAlign: 'center',marginTop:"-4px",marginBottom:"-6px" }}>
              {user?.name || "Unknown User"}
              {/* {user?.name || "Unknown User"} {user?.available ? '(Available)' : '(Unavailable)'} */}
            </h3>
            </div>
              ...
              {sortedTickets.length > 0 ? (
                sortedTickets.map((ticket) => (
                  <TicketCard
                    ...
                    title={ticket.title}
                    type={ticket.tag[0]}
                    priority={priorityLables[ticket.priority]}
                    todo={ticket.status}
                  />
                ))
              ) : (
